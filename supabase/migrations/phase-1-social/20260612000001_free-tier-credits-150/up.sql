-- Migration: 20260612000001 — free-tier-credits-150 (align DB with code)
--
-- UPDATE-ONLY. This migration raises the free tier monthly credit allotment from
-- 50 → 150 to match the already-updated TypeScript constants in:
--   • backend/src/modules/credits/credit-costs.ts (PLAN_CREDIT_ALLOTMENTS.free)
--   • backend/src/config/pricing-features.ts (PLAN_CREDIT_ALLOTMENTS.free)
--   • frontend/src/config/plan-comparison.ts
--
-- What this changes:
--   • public.credit_plan_allotment(text) — pure SQL function returns 150 for 'free'
--   • public.ensure_credit_buckets(uuid, numeric) — updates p_trial_default from 50→150
--   • public.apply_credit_charge(...) — updates p_trial_default from 50→150
--   • public.backfill_credit_buckets(numeric) — updates p_trial_default from 50→150
--
-- NOTES:
--   • EXISTING users' credit balances are NOT affected — we do NOT refund the 100
--     credit difference. New plan periods going forward will receive 150.
--   • Trial bucket semantics unchanged (14-day expiry).
--   • All other tiers unchanged: standard=500, pro=2000, ultimate=10000.
