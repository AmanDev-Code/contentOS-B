...
export class SubscriptionService {
  ...
  async getBillingInfo(userId: string): Promise<BillingInfo> {
    ...
    const usage = {
      ...
      remainingCredits: quotaData.remaining_credits || subscription.creditsLimit,
      percentageUsed: parseFloat(quotaData.percentage_used || '0'),
      resetDate: quotaData.reset_date,
    };

    const billing = {
      nextBillingDate: subscription.subscriptionEndDate || subscription.resetDate,
      amount: subscription.billingCycle === 'yearly' ? subscription.priceYearly : subscription.priceMonthly,
      currency: 'USD',
      paymentMethod: subscription.stripeCustomerId ? 'Card ending in ****' : undefined,
    };
    ...
    try {
      const { data, error } = await this.supabaseService.getServiceClient()
        ...
        .upsert({
          ...
          subscription_end_date: billingCycle === 'yearly' 
