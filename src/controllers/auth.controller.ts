...
export class AuthController {
   ...
   * Returns { url } — the Google consent page URL generated by Supabase.
   * The frontend should redirect the browser to this URL.
   */
  @Get('google')
  async initiateGoogleOAuth() {
    try {
      return await this.authService.initiateGoogleOAuth();
    } catch (error) {
      this.logger.error(
        'Google OAuth initiate error:',
  ...
  /**
   * Google OAuth callback — Supabase redirects here after the user grants consent.
   * Exchanges the authorisation code for a session and redirects the browser to
   * the frontend with the session tokens in the query string.
   */
  @Get('google/callback')
  @Redirect()
  async handleGoogleCallback(
    ...
    if (oauthError || !code || !state) {
      this.logger.warn(
        `Google OAuth callback received error or missing params: error=${oauthError}`,
      );
      return { url: `${frontendUrl}/auth?error=google_auth_failed` };
    }

    try {
      const session = await this.authService.handleGoogleOAuthCallback(
        code,
        state,
      );
  ...
  /**
   * Google sign-in for native mobile clients.
   * Accepts a Google ID token (from Google Sign-In SDK), verifies it with
   * Supabase, and returns session tokens directly.
   */
  @Post('google/mobile')
  async handleGoogleMobile(@Body() body: { id_token: string }) {
    try {
      if (!body.id_token) {
        return { success: false, message: 'id_token is required' };
      }
      const session = await this.authService.verifyGoogleIdToken(body.id_token);
      return {
        success: true,
        access_token: session.access_token,
        refresh_token: session.refresh_token,
