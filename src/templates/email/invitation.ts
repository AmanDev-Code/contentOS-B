...
export function getInvitationTemplate(inviterName: string, inviteUrl: string): string {
  const content = `
    <h1 class="title">You are invited to join Postra</h1>
    <p class="text"><strong>${inviterName}</strong> has invited you to join Postra, an AI-powered content creation platform.</p>
    <p class="text">Postra helps creators generate engaging content for LinkedIn and other social platforms. Join thousands of users who are already using AI to grow their audience.</p>
  ...
  `;
  return wrapEmail(content, 'Invitation to Postra');
}
