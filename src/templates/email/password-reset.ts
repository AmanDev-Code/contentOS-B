...
export function getPasswordResetTemplate(resetUrl: string): string {
  const content = `
    ...
    <p class="text">We received a request to reset the password for your Postra account. Click the button below to create a new password.</p>
  ...
  `;
  return wrapEmail(content, 'Reset Your Password - Postra');
}
